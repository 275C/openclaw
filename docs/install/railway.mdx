...
- `OPENROUTER_API_KEY` (required for chat — your [OpenRouter](https://openrouter.ai/) API key)
- `OPENCLAW_STATE_DIR=/data/.openclaw` (recommended)
- `OPENCLAW_WORKSPACE_DIR=/data/workspace` (recommended)
- `OPENCLAW_CONFIG_PATH=/app/deploy/railway/openclaw.json`
- `GITHUB_TOKEN` (optional — needed for cloning private GitHub repos; use a fine-grained token scoped only to required repos; never paste it into chat; store only as a Railway service variable)

<Tip>
  The bundled config at `deploy/railway/openclaw.json` pins `agents.defaults.model.primary` to
  `openrouter/auto` (best available model via OpenRouter),
  `agents.defaults.tools.exec.timeoutSec: 60` (60-second timeout for exec commands to prevent hangs),
  `gateway.controlUi.trustTokenAuth: true` to skip manual device pairing when the gateway
  token/password is provided, and `gateway.trustedProxies` for the Railway network. Set
  `OPENROUTER_API_KEY` in your Railway service variables. If you use a custom domain, edit that file
  or create your own config with the correct origin.
</Tip>
