...
<Tip>
  The bundled config at `deploy/railway/openclaw.json` sets `agents.defaults.model.primary` to
  `openrouter/auto` so agents use OpenRouter by default, `gateway.controlUi.trustTokenAuth: true` to
  skip manual device pairing when the gateway token/password is provided, and
  `gateway.trustedProxies` for the Railway network. Set `OPENROUTER_API_KEY` in your Railway service
  variables. If you use a custom domain, edit that file or create your own config with the correct
  origin.
</Tip>
